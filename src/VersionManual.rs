...
//funcion que muestra las estaciones creadas
fn mostrar_red(&self) {
        let w = 58usize;
        println!();
...
fn mostrar_diagrama(&self, ruta: Option<&Vec<usize>>) {
        ...
        // recontruye la ruta buscada para luego poder resaltara las aristas o vias de otro color.
        let ruta_aristas: std::collections::HashSet<(usize,usize)> = ruta
            .map(|r| r.windows(2).map(|w| (w[0].min(w[1]), w[0].max(w[1]))).collect())
            .unwrap_or_default();
        //resalta los vertices(estaciones) y aristas(vias) por donde paso la ruta para encontrar destino.
        let ruta_nodos: std::collections::HashSet<usize> = ruta
            .map(|r| r.iter().cloned().collect())
            .unwrap_or_default();
        //Hace resaltar de otro color la estacion de origne y la de llegada.
        let origen_idx  = ruta.map(|r| r[0]);
        let destino_idx = ruta.map(|r| *r.last().unwrap());

        //Trozo de codigo que nos sirve para poder ser llamado para dibujar las vias entre estaciones, 
        // dependiendo de la pendiente de la linea se dibuja con un caracter u otro para 
        // que quede visualmente mas claro.
        ...
        //Aqui se llama a trazar para dibujar las vias entre estaciones, 
        // se recorre la lista de adyacencia y se dibuja una linea entre 
        // cada par de estaciones conectadas, si la via esta en la ruta resaltada 
        ...
        // color tendra cada esatcion al iniciar el programa y igual buscar rutas.
        ...
        //Se crea el recuadro visual mente que se ve el diagrama la parte de arriba y
        //  tambien la primera linea que enicerrar el diagrama
        ...
        //Aqui se crea la parta donde muestra que descripcion del diagrama.
        // ── Leyenda ──────────────────────────────────────────────
...
println!("=================================");
    ...
    //llamamos a la funcion para crear estaciones
    ...
    //llamamos a la funcion para crear vias entre estaciones
    ...
    //llamamos a las funcion mostar esatciones creadas
    red.mostrar_red();
